...
- 最近一轮（0.7.4 之后、**未发版**）：① **更新说明的 Markdown 渲染**——用户反馈「更新说明无法渲染」，弹窗里以前直接显示 CHANGELOG 的 Markdown 原文（`### Fixed`、`**粗体**`），现在由 `update-notes.ts` 渲染成受控子集的安全 HTML（见「自动更新数据流」末尾 + 第 26 组验收）；② **界面缩放的加固**——用户反馈「放大根本没用，缩小有用」，滚轮监听挪到 window 捕获阶段、调档后再确认一次、没生效就在状态栏说出来（现象只在真机 WebView2 上，Chromium 复现不出来；详见「界面缩放」那节的四条注解）。
...
BROWSER_CHECK_PORT=1425 node scripts/browser-check/wysiwyg.mjs        # 101 项交互验收 + 截图
...
- **两套 UI（仿 Typora）**：`viewMode: "write" | "source"`（取代旧的 `livePreview` 布尔，旧存档自动迁移）。
  ...
  - **界面缩放（Ctrl+滚轮）**：整个界面等比放大/缩小（**不是**分栏宽度——用户反馈「字太小看不清」要的是字变大；分栏宽度那套手势已移除）。范围 **50%~250%**、一次一格 10%，默认 100%。纯逻辑在 `zoom.ts`（可单测），页面侧是 `handleZoomWheel`/`setUiZoom`/`applyUiZoom`，**走 Tauri 的 webview 缩放 `setZoom`**（权限 `core:webview:allow-set-webview-zoom`）。
    ...
    - **没生效就说出来**：`verifyZoomApplied` 复核 `devicePixelRatio`（Chromium 的 dpr 随页面缩放一起变，WebView2 的 ZoomFactor 就是页面缩放），"请求了不同系数、dpr 却纹丝不动"时状态栏给「界面缩放未生效：webview 没接受 110%…」+ 调试日志。浏览器开发桩的 `setZoom` 是假的（不改 dpr），桩在 `app.html` 里挂了 `window.__browserDevStub = { fakeZoom: true }`，页面据此跳过复核——**用标记而不是 import 桩**（import 会把 dev-only 的桩打进生产包）。
