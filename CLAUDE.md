...
- **工作区里已改、尚未发版（2026-09-18）**：**预览栏底部的横向滚动条**（用户反馈「为什么预览框还是会出现下方的滑动条」，他选定「A. 永不横滚」）。成因：他那份文档自带 `#set page(paper: "a4", margin: …, numbering: "1")` → 我们注入的「按栏宽重排」被覆盖 → 走「固定版心 + 等比缩放」老路，界面放大时 webview 缩放把 **CSS 视口一起缩小**（1400px 窗口在 150% 下只有 933 CSS px、预览栏 685 → 451px），画布被自然尺寸（A4 ≈ 568px）封顶 ⇒ 画布比栏宽还宽 ⇒ 底部横条（真浏览器实测 150% 溢出 117px、250% 溢出 304px）；而画布在 568px 就已不再变大 —— 这条横条后期什么也没换来。修法：`previewScale` 的画布宽度取 `min(缩放前栏宽, 当前栏宽, 自然尺寸)`，**恒 ≤ 栏宽**（见「界面缩放」那节）。**要不要发版、发哪个号由用户说**（红线 11）。
