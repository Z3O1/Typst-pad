...
- **0.7.7 之后、尚未发版的四件事**（都已在 `main` 上，**下次发版时补进 CHANGELOG 的 Added/Changed/Fixed，不要回头改 0.7.7 那一段**）：① 状态栏**错误计数移到警告左边**（按用户参照图 `⊗ 0 ⚠ 0`，与 VS Code 同序）；② 两个计数图标改成**内联 SVG**、比例照参照图量出来；③ **启动时的自动更新检查不再被 6 小时节流拦掉**（用户报「自动更新没法用」，原话是「打开的时候没有自动更新，但是检查的时候能检查到」——根因是启动检查被上次检查的时间戳拦住了）；④ **点过「稍后」就再也别自动弹更新窗，直到手动点「检查更新…」**（用户原话「算了，不更新就再也别跳出来，直到点了检查更新」）。③④ 合并成了同一条规则，详见「自动更新数据流」那两条红线与验收第 34 组。
...
BROWSER_CHECK_PORT=1425 node scripts/browser-check/wysiwyg.mjs        # 161 项交互验收 + 截图
