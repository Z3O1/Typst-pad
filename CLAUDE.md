...
- **0.7.8 之后、尚未发版**（都在 `main` 上，**下次发版时补进 CHANGELOG，不要回头改已发布的版本段**）：① **预览按栏宽重新排版**——用户报「预览模式还是有横的拖动的条」，最终选定"纸张跟着预览栏走、正文重排、字号不变"（见「界面缩放」那节，含前三版为什么都不行）；② **缩放复核换判据（CSS 布局宽度）+ 多量几次 + 把实测数据写进状态栏**——用户第三次反馈「缩放到最大后无法从 Ctrl+滚轮缩小」（旧判据 `devicePixelRatio` 在真机上不跟随 ZoomFactor → 复核被整体关掉 → 档位又冲上限），第四次直接截图「引擎把 150% 限制在 100%」（见「界面缩放」那节；**这一条还没在用户那台机器上确认修好**）；③ `5eb669e` 压掉 `codemirror-lang-typst` 给标题正文加的下划线（写作模式观感）。
...
BROWSER_CHECK_PORT=1425 node scripts/browser-check/wysiwyg.mjs        # 176 项交互验收 + 截图
