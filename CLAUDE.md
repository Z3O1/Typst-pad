...
- 版本 `0.7.6`（0.7.5 之后的修复版：**公式下标不再被裁** + **大缩放下状态栏不再折行** + 引擎的 `ResizeObserver loop` 提示不再误报成脚本错误）。回看：**0.7.4 是"自动更新真的能生效"的第一个版本**（0.7.3 引入 updater 但仓库当时私有，客户端拉不到清单；仓库公开 + 0.7.4 发布之后这条链路才第一次跑通）。`main` 与 `origin/main` 同步；每次打 tag 后 `release.yml` 建草稿 Release，**由后台一次性任务自动 Publish**（不必等、不必问）。**发版后的验收就照这两条 curl 做**（见下）。
- **发行状态（2026-09-14 实测）：`v0.7.5`、`v0.7.4`、`v0.7.3`、`v0.7.2`、`v0.7.0` 已发布；只有 `v0.7.1` 还是草稿**（handover 里曾把 v0.7.2 误记成草稿）；`v0.7.6` 是本轮新打的 tag，草稿由后台任务 Publish。发版时 `release.yml` 建的仍是**草稿**，**必须手动 Publish**（或按下方约定直接发）——草稿资产不对外，客户端拉不到 `latest.json`，自动更新不会生效。
...
- **未发版（在 `main` 上，等用户发话）**：① **源码模式自动换行 `Alt+Z`**（用户要求）——源码模式双栏下编辑区窄，长行只能横向滚动看行尾；② **写作模式始终自动折行**（用户要求「文档模式的内容不应该有横向拖动」；改前一条长行带来 2855px 横向滚动）；③ **预览跟着界面缩放一起变大**（用户三轮反馈后的最终结论：预览框与里面的字都要变大）——拟合按缩放前的栏宽算；代价是放大超过栏宽时预览栏会出现横向滚动条（固定版心页面二选一），并配套修好了"溢出时左缘被顶出滚动区、永远看不到"。④ **输入 `$` 自动配对**（用户要求「加入功能：自动补全 $$」）——独占一行补行间脚手架 `$  $`、行内补 `$x$`、右侧已有闭合符则跳过、空配对退格一次删净。⑤ **状态栏最左加「警告」计数**（用户要求，VS Code 风格的三角形感叹号图标、排在错误计数左边、两者一起移到最左边，且都常驻显示）。⑥ **切换模式保持光标位置**（用户要求）—— CodeMirror 的滚动锚点是「最上面那条可见行」，换整套布局后光标会被甩出视口（实测 y=415 → 920，视口只有 800），现在按光标的屏幕高度锚回去。细节见「两套 UI」「所见即所得」两节的注解与验收第 29/30/31/32/33 组。**这六笔都没有进 CHANGELOG**：本仓库的 CHANGELOG 是在发版时按版本号写段的（红线 11），发 0.7.7 时从提交里补即可。
...
BROWSER_CHECK_PORT=1425 node scripts/browser-check/wysiwyg.mjs        # 147 项交互验收 + 截图
...
- **两套 UI（仿 Typora）**：`viewMode: "write" | "source"`（取代旧的 `livePreview` 布尔，旧存档自动迁移）。
  ...
  - **界面缩放（Ctrl+滚轮）**：整个界面等比放大/缩小（**不是**分栏宽度——用户反馈「字太小看不清」要的是字变大；分栏宽度那套手势已移除）。范围 **50%~250%**、一次一格 10%，默认 100%。纯逻辑在 `zoom.ts`（可单测），页面侧是 `handleZoomWheel`/`setUiZoom`/`applyUiZoom`，**走 Tauri 的 webview 缩放 `setZoom`**（权限 `core:webview:allow-set-webview-zoom`）。
    ...
    - **状态栏最左是「警告 / 错误」计数**（用户要求：「加入警告，用类似 vscode 的图标（三角形内部有感叹号），放到错误数量左边。另外，把这两个东西都移到最左边」）：最左是一个 `.badge-group`，**警告在左、错误在右**，两者都**常驻显示**（无问题时是 0，与 VS Code 状态栏一致；常驻还能避免数字出现/消失时整条状态栏左右抖动），状态文字排在其后。
      - **警告图标是内联 SVG**（三角形描边 + 感叹号，`viewBox="0 0 16 16"`，`currentColor` 上色）：**不要换回 `⚠` 字形** —— 它跨字体渲染差异大，而且常常被彩色 emoji 字体接管，与旁边 CSS 圆环 + `✕` 的错误图标不是一套观感。两条描边路径 + 一个圆点；验收里断言"有图形元素且图标内没有文字内容"。
