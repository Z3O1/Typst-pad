...
- **未发版（在 `main` 上，等用户发话）**：① **源码模式自动换行 `Alt+Z`**（用户要求）——源码模式双栏下编辑区窄，长行只能横向滚动看行尾；② **写作模式始终自动折行**（用户要求「文档模式的内容不应该有横向拖动」；改前一条长行带来 2855px 横向滚动）；③ **预览栏永不横向拖动**（用户要求）——界面缩放**不会**让预览变大，这是"不许横向拖动"的必然代价（中间试过"预览跟着缩放放大"的一版，250% 下预览栏溢出 166px，被用户否掉并已改回）。④ **输入 `$` 自动配对**（用户要求「加入功能：自动补全 $$」）——独占一行补行间脚手架 `$  $`、行内补 `$x$`、右侧已有闭合符则跳过、空配对退格一次删净。细节见「两套 UI」「所见即所得」两节的注解与验收第 29/30/31 组。**这四笔都没有进 CHANGELOG**：本仓库的 CHANGELOG 是在发版时按版本号写段的（红线 11），发 0.7.7 时从提交里补即可。
...
BROWSER_CHECK_PORT=1425 node scripts/browser-check/wysiwyg.mjs        # 133 项交互验收 + 截图
...
src/lib/preview-scale.ts    # 预览画布等宽缩放：pt → px（1pt = 4/3px），字号对齐编辑区；只用实测栏宽（画布恒 ≤ 栏宽 = 预览栏永不横向拖动）
...
- **两套 UI（仿 Typora）**：`viewMode: "write" | "source"`（取代旧的 `livePreview` 布尔，旧存档自动迁移）。
  ...
  - **界面缩放（Ctrl+滚轮）**：整个界面等比放大/缩小（**不是**分栏宽度——用户反馈「字太小看不清」要的是字变大；分栏宽度那套手势已移除）。范围 **50%~250%**、一次一格 10%，默认 100%。纯逻辑在 `zoom.ts`（可单测），页面侧是 `handleZoomWheel`/`setUiZoom`/`applyUiZoom`，**走 Tauri 的 webview 缩放 `setZoom`**（权限 `core:webview:allow-set-webview-zoom`）。
    ...
    - **预览栏永不横向拖动 —— 别再把界面缩放乘进预览的拟合**（2026-09-14 两轮反馈，方向相反，最终以"不许横向拖动"为准）：预览画布是**自适应铺满**的（`applyPreviewScale` → `preview-scale.ts`），而界面缩放走 webview `setZoom`，它把 CSS 视口一起缩小（预览栏 CSS 宽度 505 → 331）。
      - 第一轮反馈「代码模式预览框的缩放还是无效 / 变了但立刻弹回原样」：拿缩小后的栏宽算铺满 → 画布被缩回原样、与引擎的放大正好抵消。实测：1040px 窗口下 100%→150% 画布**物理尺寸比 0.983**（等于没变），1258px 下只长 6.7%（期望 50%）。
      - 于是改成"按缩放**前**的栏宽拟合"（把 `uiZoom` 乘进容器宽度）：缩放确实带动了预览（物理比 1.500），**但固定版心的页面塞不进栏宽只能横向滚动** —— 实测 250% 时预览栏横向溢出 **166px**，用户明确否掉：「预览模式和文档模式的内容不应该有横向拖动，而是自动换行，Alt+Z 只对代码起效」。
      - **现在的规则**：预览拟合**永远只用实测栏宽**，画布宽度恒 ≤ 栏宽（`preview-scale.test.ts` 有一组"永不横向溢出"回归用例，覆盖 120~2400px 的栏宽）。代价必须记住：**界面缩放不会让预览内容变大**（预览始终正好塞进预览栏）——固定版心的排版结果在"永不横向拖动"与"跟着缩放变大"之间只能二选一，用户选了前者。想改回去就是那一处乘法，但先问用户。
      - 验收第 30 组（4 项）用 CDP `Emulation.setDeviceMetricsOverride` 把 CSS 视口压到「窗口宽 ÷ 缩放」造 100%/150%/250%：三档都断言画布 ≤ 栏宽且无横向溢出，并断言 250% 时画布跟着变窄的栏宽走（不再保持 100% 时的宽度）。 → **第 31 组 `$` 自动配对**（8 项）：空行输入 `$` 补出 `$  $`、接着敲字得到 `$ x $`、光标移开后渲染成**块级** widget（桩收到 `display:true`）、行内补成 `前文 $y$`、连按两下 `$` 不产生多余一对、退格一次整对删、代码区里只插入一个 `$`。
