...
- 版本 `0.7.7`（0.7.6 之后的**输入与阅读手感**版：**`$` 自动配对** + **写作模式始终折行** + **预览跟着界面缩放一起变大** + **状态栏最左的警告/错误计数** + **切换模式不再移动光标**）。回看：**0.7.4 是"自动更新真的能生效"的第一个版本**（0.7.3 引入 updater 但仓库当时私有，客户端拉不到清单；仓库公开 + 0.7.4 发布之后这条链路才第一次跑通）。`main` 与 `origin/main` 同步；每次打 tag 后 `release.yml` 建草稿 Release，**由后台一次性任务自动 Publish**（不必等、不必问）。**发版后的验收就照这两条 curl 做**（见下）。
- **发行状态（2026-09-14 实测）：`v0.7.6`、`v0.7.5`、`v0.7.4`、`v0.7.3`、`v0.7.2`、`v0.7.0` 已发布；只有 `v0.7.1` 还是草稿**（handover 里曾把 v0.7.2 误记成草稿）；`v0.7.7` 是本轮新打的 tag，草稿由后台任务 Publish。发版时 `release.yml` 建的仍是**草稿**，**必须手动 Publish**（或按下方约定直接发）——草稿资产不对外，客户端拉不到 `latest.json`，自动更新不会生效。
...
- **0.7.7 之后、尚未发版的三件事**（都已在 `main` 上，**下次发版时补进 CHANGELOG 的 Added/Changed/Fixed，不要回头改 0.7.7 那一段**）：① 状态栏**错误计数移到警告左边**（按用户参照图 `⊗ 0 ⚠ 0`，与 VS Code 同序）；② 两个计数图标改成**内联 SVG**、比例照参照图量出来；③ **启动时的自动更新检查不再被 6 小时节流拦掉**（用户报「自动更新没法用」，原话是「打开的时候没有自动更新，但是检查的时候能检查到」——根因是启动检查被上次检查的时间戳拦住了，详见「自动更新数据流」那条红线与验收第 34 组）。
...
BROWSER_CHECK_PORT=1425 node scripts/browser-check/wysiwyg.mjs        # 154 项交互验收 + 截图
...
src/lib/preview-scale.ts    # 预览画布等宽缩放：pt → px（1pt = 4/3px），字号对齐编辑区；拟合按**缩放前**的栏宽算（收 uiZoom），让预览跟着界面缩放一起变大
...
- **输入 `$` 自动配对**（用户要求「加入功能：自动补全 $$」，2026-09-14）：敲一个 `$` 就把定界符补成一对、光标落在中间，判定全在 `auto-pair.ts`（纯函数可单测），落事务在 `Editor.svelte` 的 `EditorView.inputHandler`（只在"空选区 + 输入内容恰好是 `$`"时介入，不碰粘贴 / IME / 选中替换；任何抛错都 `return false` 退回默认输入，绝不吞按键）。
  ...
  - **不配对的五类上下文**：代码 / raw / 注释 / 字符串里；**代码区的紧邻右边界**（`#let s = 1|` 这种"还在写代码"的位置——`regionAt` 是左闭右开，区域末端本身已不算代码，所以单独挡一道）；已有公式内部（`$a + |b$`，那里的 `$` 是闭合公式）；前面是反斜杠（`\$`）；位置越界。已知代价：`#f(1)|$x$` 这种"代码表达式后紧跟公式"也要手打闭合符——按"宁可漏配对，不可误配对"取向接受。
...
- **两套 UI（仿 Typora）**：`viewMode: "write" | "source"`（取代旧的 `livePreview` 布尔，旧存档自动迁移）。
  ...
  - **界面缩放（Ctrl+滚轮）**：整个界面等比放大/缩小（**不是**分栏宽度——用户反馈「字太小看不清」要的是字变大；分栏宽度那套手势已移除）。范围 **50%~250%**、一次一格 10%，默认 100%。纯逻辑在 `zoom.ts`（可单测），页面侧是 `handleZoomWheel`/`setUiZoom`/`applyUiZoom`，**走 Tauri 的 webview 缩放 `setZoom`**（权限 `core:webview:allow-set-webview-zoom`）。
    ...
    - **预览必须跟着界面缩放一起变大**（2026-09-14 三轮反馈的最终结论；用户原话依次是「代码模式预览框的缩放还是无效 / 变了但立刻弹回原样」→「预览模式和文档模式的内容不应该有横向拖动」→「预览框大小还是没变 / 预览框里面的字的大小还是没变」）：预览画布是**自适应铺满**的（`applyPreviewScale` → `preview-scale.ts`），而界面缩放走 webview `setZoom`，它把 CSS 视口一起缩小（预览栏 CSS 宽度 505 → 331）。
      - **根因**：拿缩小后的栏宽算"铺满"，画布就缩回原样、与引擎的放大正好抵消 —— 缩放对编辑区有效（字号是 CSS px，被引擎放大）、对预览无效。实测（不修）：1040px 窗口下 100%→150% 画布**物理尺寸比 0.983**（等于没变）。
      - **现在的规则**：`previewCanvasWidth` 收 `uiZoom`，拟合按**缩放前**的栏宽算（`containerWidth × uiZoom`，非法值按 1）→ 画布的 CSS 宽度保持在 100% 时的值，由引擎把它真正放大（1.5 档就是 1.5 倍，**页面与里面的字一起变大**）。附带好处：每一档缩放下预览字号都与编辑区一致（11pt 正文 ↔ 14px 编辑区）。
      - **必然的代价（别再当成 bug 修回去）**：页面是**固定版心**的排版结果，放大到超过栏宽时预览栏**会出现横向滚动条**。"跟着缩放变大"与"永不横向滚动"对固定版心的页面**只能二选一**——中间那版"画布恒 ≤ 栏宽"确实满足"不横向滚"，但等于界面缩放完全不影响预览，被用户否掉。想要两者兼得只能按栏宽**重新排版**（预览就不再是"整页分页对照"、也不再对应导出的 PDF，没做）。
      - **配套：溢出时左缘必须可达**。画布居中改成 `.preview-paper { margin-inline: auto }`（不再依赖容器 `align-items: center`）：auto 外边距在**负剩余空间**下会退化成 0 →"装得下就居中、装不下就左对齐"；靠容器居中会把页面的左半部分顶到滚动区之外，而 `scrollLeft` 不能为负 → 那部分**永远看不到**（实测踩过）。
      - 验收第 30 组（5 项）用 CDP `Emulation.setDeviceMetricsOverride` 把 CSS 视口压到「窗口宽 ÷ 缩放」造 100%/150%/250%：100% 照旧铺满窄栏、150% 物理尺寸放大 1.5 倍且 CSS 不再缩水、250% 继续放大且确实比栏宽大（要横向滚）、**溢出时左缘偏移 ≥ 0（可达）**。
    - **状态栏最左是「错误 / 警告」计数**（用户要求：「加入警告，用类似 vscode 的图标（三角形内部有感叹号）…另外，把这两个东西都移到最左边」，随后又给了参照图 **⊗ 0 ⚠ 0** 定序）：最左是一个 `.badge-group`，**错误在左、警告在右**（与 VS Code 状态栏同序），两者都**常驻显示**（无问题时是 0，与 VS Code 状态栏一致；常驻还能避免数字出现/消失时整条状态栏左右抖动），状态文字排在其后。
      ...
      - **这一改发生在 0.7.7 发布之后**（0.7.7 的安装包里仍是"警告在左"，tag 不追改）：下次发版时要在 CHANGELOG 里补一条 Changed（「错误计数移到警告左边，与 VS Code 状态栏同序」），**不要回头去改 0.7.7 那一段**。
...
- **每次启动都检查一次**：`+page.svelte` 的 mount 里按 `autoCheckUpdates` 开关决定要不要挂一个 4 秒延时任务，**没有别的门**。这里曾经有一道"距上次检查满 6 小时才查"的节流（`AUTO_CHECK_MIN_INTERVAL_MS` + `isCheckDue`），2026-09-14 用户报「自动更新没法用」就是它造成的：那次启动的检查被安装 0.7.6 时写下的时间戳拦掉了，于是打开应用永远不提示新版本，**只有手动点「检查更新」才查得到**（用户原话「打开的时候没有自动更新，但是检查的时候能检查到」）——而且手动检查也会刷新那个时间戳，越手动查越不会自动查。**别再把它加回来**；`lastUpdateCheckAt` 现在只是一条记录（诊断用：能看出上次检查是什么时候）。
  - 这条路径的回归网是验收**第 34 组**（`wysiwyg.mjs`）：把 `lastUpdateCheckAt` 种成"几十秒前"（= 刚检查过）再用 `&fakeupdate=1` 打开页面，断言**什么都不点**弹窗也会自己出现；并断言关掉设置开关后**一次都不查**（桩把调用次数记在 `window.__browserDevUpdaterChecks`）。这条在浏览器里能验，是因为桩能伪造"有可用更新"——真机上的下载/签名/装包仍然只能在桌面版验。
