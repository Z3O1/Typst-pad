...
- **未发版（在 `main` 上，等用户发话）**：① **源码模式自动换行 `Alt+Z`**（用户要求）——源码模式双栏下编辑区窄，长行只能横向滚动看行尾；② **写作模式始终自动折行**（用户要求「文档模式的内容不应该有横向拖动」；改前一条长行带来 2855px 横向滚动）；③ **预览跟着界面缩放一起变大**（用户三轮反馈后的最终结论：预览框与里面的字都要变大）——拟合按缩放前的栏宽算；代价是放大超过栏宽时预览栏会出现横向滚动条（固定版心页面二选一），并配套修好了"溢出时左缘被顶出滚动区、永远看不到"。④ **输入 `$` 自动配对**（用户要求「加入功能：自动补全 $$」）——独占一行补行间脚手架 `$  $`、行内补 `$x$`、右侧已有闭合符则跳过、空配对退格一次删净。细节见「两套 UI」「所见即所得」两节的注解与验收第 29/30/31 组。**这四笔都没有进 CHANGELOG**：本仓库的 CHANGELOG 是在发版时按版本号写段的（红线 11），发 0.7.7 时从提交里补即可。
...
BROWSER_CHECK_PORT=1425 node scripts/browser-check/wysiwyg.mjs        # 134 项交互验收 + 截图
