...
- **0.7.8 之后、尚未发版**（都在 `main` 上，**下次发版时补进 CHANGELOG，不要回头改已发布的版本段**）：① **预览按栏宽重新排版**——用户报「预览模式还是有横的拖动的条」，最终选定"纸张跟着预览栏走、正文重排、字号不变"（见「界面缩放」那节，含前三版为什么都不行）；② `5eb669e` 压掉 `codemirror-lang-typst` 给标题正文加的下划线（写作模式观感）。
...
BROWSER_CHECK_PORT=1425 node scripts/browser-check/wysiwyg.mjs        # 170 项交互验收 + 截图
