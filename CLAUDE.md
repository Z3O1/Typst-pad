...
- **0.7.8 之后、尚未发版**（都在 `main` 上，**下次发版时补进 CHANGELOG，不要回头改已发布的版本段**）：① **预览按栏宽重新排版**——用户报「预览模式还是有横的拖动的条」，最终选定"纸张跟着预览栏走、正文重排、字号不变"（见「界面缩放」那节，含前三版为什么都不行）；② **缩放复核的判据换成 CSS 布局宽度**——用户第三次反馈「缩放到最大后无法从 Ctrl+滚轮缩小」，根因是旧判据（`devicePixelRatio`）在真机上不跟随 ZoomFactor → 复核被整体关掉 → 档位又冲上限（见「界面缩放」那节）；③ `5eb669e` 压掉 `codemirror-lang-typst` 给标题正文加的下划线（写作模式观感）。
...
BROWSER_CHECK_PORT=1425 node scripts/browser-check/wysiwyg.mjs        # 173 项交互验收 + 截图
...
- **两套 UI（仿 Typora）**：`viewMode: "write" | "source"`（取代旧的 `livePreview` 布尔，旧存档自动迁移）。
  ...
  - **界面缩放（Ctrl+滚轮）**：整个界面等比放大/缩小（**不是**分栏宽度——用户反馈「字太小看不清」要的是字变大；分栏宽度那套手势已移除）。范围 **50%~250%**、一次一格 10%，默认 100%。纯逻辑在 `zoom.ts`（可单测），页面侧是 `handleZoomWheel`/`setUiZoom`/`applyUiZoom`，**走 Tauri 的 webview 缩放 `setZoom`**（权限 `core:webview:allow-set-webview-zoom`）。
    ...
    - **引擎不接受的档位不许记进状态**（2026-09-14 **三轮**实机反馈「放大根本没用，缩小有用」→「最大后无法用滚轮缩小」→「缩放到最大后无法从 Ctrl+滚轮缩小」是**同一个机制**：引擎没接受放大，而 `uiZoom` 照旧涨到上限 250%，于是从 250% 往下滚要滚十几档才有反应，看着就是「缩不了」）。`verifyZoomApplied` 的做法：① 首次应用缩放前 `ensureZoomCalibration` 先设 100%（排掉 WebView2 记住上次站点缩放的干扰）并记下**此时的 CSS 布局宽度**当基准；② 每次调档（含"再确认一次"那一拍）后按 **`100% 基准宽度 ÷ 当前布局宽度`** 反推**引擎实际接受的档位**（见 `zoom.ts` 的 `zoomFromWidths`）；③ 与请求值不一致就把 `uiZoom` **拉回引擎给的档位**，状态栏写明「界面缩放未生效：引擎把 220% 限制在 210%」。放大被拒时档位原地不动、界面与状态一致，缩小立刻有效；`uiZoom` 被赋值会再触发一次 `$effect`，但那时 `applied == target` 会直接返回，不会循环。
