...
- **未发版（在 `main` 上，等用户发话）**：① **源码模式自动换行 `Alt+Z`**（用户要求）——源码模式双栏下编辑区窄，长行只能横向滚动看行尾；② **写作模式始终自动折行**（用户要求「文档模式的内容不应该有横向拖动」；改前一条长行带来 2855px 横向滚动）；③ **预览栏永不横向拖动**（用户要求）——界面缩放**不会**让预览变大，这是"不许横向拖动"的必然代价（中间试过"预览跟着缩放放大"的一版，250% 下预览栏溢出 166px，被用户否掉并已改回）。细节见「两套 UI」两节的注解与验收第 29/30 组。**这三笔都没有进 CHANGELOG**：本仓库的 CHANGELOG 是在发版时按版本号写段的（红线 11），发 0.7.7 时从提交里补即可。
...
BROWSER_CHECK_PORT=1425 node scripts/browser-check/wysiwyg.mjs        # 125 项交互验收 + 截图
...
- **两套 UI（仿 Typora）**：`viewMode: "write" | "source"`（取代旧的 `livePreview` 布尔，旧存档自动迁移）。
  ...
  - **界面缩放（Ctrl+滚轮）**：整个界面等比放大/缩小（**不是**分栏宽度——用户反馈「字太小看不清」要的是字变大；分栏宽度那套手势已移除）。范围 **50%~250%**、一次一格 10%，默认 100%。纯逻辑在 `zoom.ts`（可单测），页面侧是 `handleZoomWheel`/`setUiZoom`/`applyUiZoom`，**走 Tauri 的 webview 缩放 `setZoom`**（权限 `core:webview:allow-set-webview-zoom`）。
    ...
    - **预览栏永不横向拖动 —— 别再把界面缩放乘进预览的拟合**（2026-09-14 两轮反馈，方向相反，最终以"不许横向拖动"为准）：预览画布是**自适应铺满**的（`applyPreviewScale` → `preview-scale.ts`），而界面缩放走 webview `setZoom`，它把 CSS 视口一起缩小（预览栏 CSS 宽度 505 → 331）。
      ...
      - 验收第 30 组（4 项）用 CDP `Emulation.setDeviceMetricsOverride` 把 CSS 视口压到「窗口宽 ÷ 缩放」造 100%/150%/250%：三档都断言画布 ≤ 栏宽且无横向溢出，并断言 250% 时画布跟着变窄的栏宽走（不再保持 100% 时的宽度）。
