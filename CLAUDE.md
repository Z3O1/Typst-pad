...
- **发行状态（2026-09-18 更新）：`v0.8.2` 已发布并匿名验证过（2026-09-18）**：`latest.json` 200 / `version: 0.8.2` / 安装包 200（**16698870 字节**，`Typst-pad_0.8.2_x64-setup.exe`）/ **验签通过**（keyid `4e79c0dd71347bb5`、alg `ED`）。**注意安装包比 0.8.1 小了 ~10 万字节 —— 那正是被剥掉的 wasm**（`typst_syntax_bg.wasm`）。流程同上一版：tag 推上去后由后台一次性任务盯草稿 → 02:36:22 就绪、两秒内 Publish → 立即匿名验收（退出码 0），**全程不阻塞回合**。复验：`node scripts/verify-release.mjs 0.8.2`。上一版 **`v0.8.1`（2026-09-17）**：`latest.json` 200 / `version: 0.8.1` / 安装包 **16800426 字节** / 验签通过（同 keyid）。再上一版 **`v0.8.0`（2026-09-16）**：安装包 **16806820 字节** / 验签通过；`isDraft` 为 `false`，资产齐全（latest.json / exe / exe.sig / msi / msi.sig）。旧版本清单：`v0.8.1`、`v0.8.0`、`v0.7.10`、`v0.7.9`、`v0.7.8`、`v0.7.7`、`v0.7.6`、`v0.7.5`、`v0.7.4`、`v0.7.3`、`v0.7.2`、`v0.7.0` 已发布；只有 `v0.7.1` 还是草稿**（handover 里曾把 v0.7.2 误记成草稿）。**0.7.10 已匿名验证过（2026-09-15）**：`latest.json` 200 / `version: 0.7.10` / 安装包 200（16772701 字节，`Typst-pad_0.7.10_x64-setup.exe`）/ **签名与配置里的公钥验签通过**（keyid `4e79c0dd71347bb5`、alg `ED`）；`gh release view` 的 `isDraft` 为 `false`，资产齐全（latest.json / exe / exe.sig / msi / msi.sig）。0.7.9 那次的记录：安装包 200（16762871 字节）/ **签名验签也通过**（keyid `4e79c0dd71347bb5` + ed25519 verify：公钥取 `plugins.updater.pubkey` 第二行，签名的 alg `ED` = 先 BLAKE2b-512 再签，node:crypto 就能验）。发版时 `release.yml` 建的仍是**草稿**，**必须手动 Publish**（或按下方约定直接发）——草稿资产不对外，客户端拉不到 `latest.json`，自动更新不会生效。
