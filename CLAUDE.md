...
- **未发版（在 `main` 上，等用户发话）**：① **源码模式自动换行 `Alt+Z`**（用户要求，2026-09-14）——源码模式双栏下编辑区窄，而 CodeMirror 默认不折行，长行只能横向滚动看行尾；② **预览画布跟随界面缩放**（用户反馈「代码模式预览框的缩放还是无效」「变了但立刻弹回原样」）——自适应铺满用的栏宽是缩放**后**量到的，画布被缩回原样、与引擎的放大正好抵消，现在按缩放前的栏宽拟合。细节见「两套 UI → 源代码模式」「界面缩放」两节的注解与验收第 29/30 组（9 + 4 项）。**这两笔都没有进 CHANGELOG**：本仓库的 CHANGELOG 是在发版时按版本号写段的（红线 11），发 0.7.7 时从提交里补即可。
...
BROWSER_CHECK_PORT=1425 node scripts/browser-check/wysiwyg.mjs        # 124 项交互验收 + 截图
...
src/lib/preview-scale.ts    # 预览画布等宽缩放：pt → px（1pt = 4/3px），字号对齐编辑区；拟合按**缩放前**的栏宽算（收 uiZoom）
...
- **两套 UI（仿 Typora）**：`viewMode: "write" | "source"`（取代旧的 `livePreview` 布尔，旧存档自动迁移）。
  ...
  - **源代码模式**（`Ctrl+/`，双栏）：等宽 14px + 行号 + oneDark（暗），右侧整页预览；此模式下 live-preview 整体关闭（看到的是真正的 Typst 源码）。
    - **自动换行 `Alt+Z`**（用户要求，2026-09-14）：源码模式是"源码 + 预览"双栏、编辑区窄，而 CodeMirror **默认不折行**——长行必须横向滚动才看得到行尾。`Alt+Z` 切换（VS Code 同款手势，菜单「视图 → 自动换行」同一入口，右侧灰字只是提示），打开时长行折行显示（`EditorView.lineWrapping` → 内容元素带 `cm-lineWrapping`，`white-space: break-spaces` + 断词）。
      ...
      - **只作用于源码模式**：传给 Editor 的是 `wrap={viewMode === "source" && editorWrap}`，所以写作模式（整页纸张、按原文排）不会跟着折行；写作模式下按 `Alt+Z` 只给提示「自动换行只在源代码模式生效（Ctrl+/ 切换）」，不改状态。开关随存档持久化（`editorWrap`，默认关 = 保持原有观感）。
  ...
  - **界面缩放（Ctrl+滚轮）**：整个界面等比放大/缩小（**不是**分栏宽度——用户反馈「字太小看不清」要的是字变大；分栏宽度那套手势已移除）。范围 **50%~250%**、一次一格 10%，默认 100%。纯逻辑在 `zoom.ts`（可单测），页面侧是 `handleZoomWheel`/`setUiZoom`/`applyUiZoom`，**走 Tauri 的 webview 缩放 `setZoom`**（权限 `core:webview:allow-set-webview-zoom`）。
    ...
    - **预览画布必须按"缩放前"的栏宽拟合**（2026-09-14 用户反馈「代码模式预览框的缩放还是无效」「Ctrl+滚轮 变了但立刻弹回原样」）：预览画布是**自适应铺满**的（`applyPreviewScale` → `preview-scale.ts`），而界面缩放走 webview `setZoom`，它把 **CSS 视口一起缩小**——预览栏的 CSS 宽度从 505 变成 331。以前直接拿这个"已缩小"的宽度算铺满，画布就缩回原样（`ResizeObserver` 在缩放后又算了一次），而引擎再放大一次恰好抵消：**缩放对编辑区有效（字号是 CSS px，被引擎放大）、对预览无效**。实测复现：1040px 窗口下 100%→150%，画布**物理尺寸比 0.983**（等于没变）；1258px 窗口下只长 6.7%（期望 50%）。修法是把 `uiZoom` 传给 `previewCanvasWidth`，按缩放**前**的栏宽算 —— 画布的 CSS 宽度保持在 100% 时的值，由引擎把它真正放大（超出栏宽就横向滚动）。修后实测物理尺寸比 **1.500**（1258px）/ **1.475**（1040px，差的 1.7% 来自不随缩放变的固定内边距）。附带好处：每一档缩放下预览字号都仍与编辑区字号一致（11pt 正文 ↔ 14px 编辑区）。
      - 验收第 30 组（4 项）用 CDP `Emulation.setDeviceMetricsOverride` 把 CSS 视口压到「窗口宽 ÷ 1.5」+ 预置 `uiZoom=1.5` 来造 150%（布局上等价于 webview 缩放，量到的 rect 仍是未放大的 CSS px，故物理尺寸 = 画布 CSS × 1.5）：100% 时照旧铺满窄栏、150% 时物理尺寸 >1.4 倍、画布 CSS 不跟着窄栏缩水、放大后比栏宽大（有横向溢出可滚）。
  ...
  - 快捷键分三处：**无 Shift 的 Ctrl 组合**（Ctrl+B/I/K/1/2/3/0/M）放菜单项的 `shortcut` 由 MenuBar 统一匹配；**带 Shift 的**（Ctrl+Shift+` 行内代码、Ctrl+Shift+M 公式块、Ctrl+Shift+[ / ] 列、Ctrl+Shift+Q 引用、Ctrl+Shift+C 代码块）由 `+page.svelte` 的 window keydown 处理——MenuBar 的匹配器只支持「Ctrl+单键」，这是既有约定（见 menu-keys.ts）；**不带 Ctrl 的 `Alt+Z`**（源码模式自动换行）同样走 `+page.svelte` 的 window keydown（判定见 word-wrap.ts）。
