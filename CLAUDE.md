...
- **顶部菜单栏的展开面板固定白底黑字（用户 2026-09-18 要求「把上方菜单栏的展开菜单改成白色」）**：`.menu-dropdown` 在 `MenuBar.svelte` 里**就地声明一组局部变量**（`--menu-bg: #ffffff` / `--menu-fg: #1f1f1f` / `--menu-fg-dim: #6b6b6b` / `--menu-hover-bg: #e8f2f9` / `--menu-hover-fg: #0b6bb5`），**不跟应用主题走** —— 深色主题下也是白底黑字（用户提这条需求时用的就是深色主题）。**别再改回 `var(--bg-toolbar)` / `var(--fg)`**：那两条在暗色主题下给的是深底浅字（`--fg` 是 `#d4d4d4`），白底配浅灰字等于看不见。悬停也从"亮蓝底 + 白字"（`--accent` + `#fff`）改成"浅蓝底 + 蓝字"——白底上旧组合对比度不够；面板变白 ⇒ 阴影从 `0.35` 压到 `rgba(0, 0, 0, 0.18)`，否则白块边缘发黑。验收第 16 组新增 4 项锁住：**浅色（自动）与深色主题下各断言实测计算样式**（面板 `rgb(255, 255, 255)`、文字 `rgb(31, 31, 31)`；深色那两条还顺带断言顶栏确实不是白的 = 主题真的切过去了）。**右键菜单（`ContextMenu.svelte`）仍是深色** —— 用户只要求了顶部菜单栏那一个，要一起改得他先说。
