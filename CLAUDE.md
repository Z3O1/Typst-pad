...
- **未发版（在 `main` 上，等用户发话）**：**源码模式自动换行 `Alt+Z`**（用户要求，2026-09-14）——源码模式双栏下编辑区窄，而 CodeMirror 默认不折行，长行只能横向滚动看行尾。细节见「两套 UI → 源代码模式」那节的注解与验收第 29 组（9 项）。**这一笔没有进 CHANGELOG**：本仓库的 CHANGELOG 是在发版时按版本号写段的（红线 11），发 0.7.7 时从提交里补即可。
...
BROWSER_CHECK_PORT=1425 node scripts/browser-check/wysiwyg.mjs        # 120 项交互验收 + 截图
