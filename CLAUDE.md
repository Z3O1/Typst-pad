...
- **0.7.8 之后、尚未发版**（都在 `main` 上，**下次发版时补进 CHANGELOG，不要回头改已发布的版本段**）：① **预览按栏宽重新排版**——用户报「预览模式还是有横的拖动的条」，最终选定"纸张跟着预览栏走、正文重排、字号不变"（见「界面缩放」那节，含前三版为什么都不行）；② **缩放复核换判据（CSS 布局宽度）+ 多量几次 + 把实测数据写进状态栏**——用户第三次反馈「缩放到最大后无法从 Ctrl+滚轮缩小」（旧判据 `devicePixelRatio` 在真机上不跟随 ZoomFactor → 复核被整体关掉 → 档位又冲上限），第四次直接截图「引擎把 150% 限制在 100%」（见「界面缩放」那节；**这一条还没在用户那台机器上确认修好**）；③ `5eb669e` 压掉 `codemirror-lang-typst` 给标题正文加的下划线（写作模式观感）；④ **Esc 退出设置界面 + `Ctrl+Shift+N` 新建窗口**（用户要求「需要按 Esc 退出设置界面 和 Ctrl + Shift + N 新建窗口」）——`Ctrl+Shift+N` **自 0.7.0 起就失效了**（判定被 Shift 格式表整段吞掉），这次是把它修回来，同时补上 Esc 关弹窗与副窗口的存档隔离（见「多窗口与页面级按键路由」）。
...
BROWSER_CHECK_PORT=1425 node scripts/browser-check/wysiwyg.mjs        # 187 项交互验收 + 截图
...
src/lib/app-keys.ts         # **页面级按键路由纯函数**：Esc 关弹窗 / Alt+Z / Ctrl+Shift+N 新建窗口 / Ctrl+Shift+格式表 / Ctrl+R / Ctrl+W（顺序敏感，见「多窗口与页面级按键路由」）（可单测）
...
- **两套 UI（仿 Typora）**：`viewMode: "write" | "source"`（取代旧的 `livePreview` 布尔，旧存档自动迁移）。
  ...
  - **界面缩放（Ctrl+滚轮）**：整个界面等比放大/缩小（**不是**分栏宽度——用户反馈「字太小看不清」要的是字变大；分栏宽度那套手势已移除）。范围 **50%~250%**、一次一格 10%，默认 100%。纯逻辑在 `zoom.ts`（可单测），页面侧是 `handleZoomWheel`/`setUiZoom`/`applyUiZoom`，**走 Tauri 的 webview 缩放 `setZoom`**（权限 `core:webview:allow-set-webview-zoom`）。
    ...
    - **预览按栏宽重新排版（0.7.8 之后的最终形态）**：2026-09-14 用户第三轮反馈「预览模式还是有横的拖动的条」，在给了几何数据（他的窗口 1379×980、缩放 210% → 预览栏只有 ~318 CSS px，而 A4 自然尺寸 568px）后他选定 **C：按栏宽重新排版**。**别再退回前两版**：
      ...
      - 验收第 30 组（10 项）用 CDP `Emulation.setDeviceMetricsOverride` 造 100%/150%/250%：页宽确实按「栏宽 × 11/14」传给后端、产物页宽 = 请求页宽、**三档缩放下横向溢出都是 0**、预览字号（`getScreenCTM().a`）恒 ≈ 14/11 且不随栏宽缩水、250% 下页数变多（真的重排了）、栏宽变化会重编译、以及 `&reflowfail=1` 时退回等比缩放。真机侧的重排本身由 Rust 单测锁住（`compile_with_page_width_*`：页宽生效、窄页页数变多、诊断行号不漂移）。
