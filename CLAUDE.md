...
Typst-pad：**仿 Typora 的 Typst 桌面编辑器，两套 UI**——「写作模式」（默认，单栏整页纸张：公式与标记就地排版、光标/选区进入即展开源码、无行号）与「源代码模式」（`Ctrl+E`，双栏：等宽代码编辑器 + 右栏整页预览）。前端 SvelteKit SPA（adapter-static），桌面壳 Tauri 2（Rust），编译渲染用**内嵌 typst crate**（0.15.x，Rust 进程内编译，本地字体）。代码注释与 README 均为中文。

## 交接须知（新同学先读这一节）

**这是什么**：仿 Typora 的 Typst 桌面编辑器 —— 「写作模式」（单栏整页纸张、公式与标记就地排版、光标进入即展开源码）与「源代码模式」（`Ctrl+E`，双栏：等宽编辑器 + 整页预览）。纯本地运行：前端 SvelteKit SPA（adapter-static），桌面壳 Tauri 2，排版引擎是**内嵌的 typst crate**（Rust 进程内编译，没有 wasm、没有网络依赖，字体随包分发）。

**交接时的状态（2026-09-18）**：

- 版本 `0.9.0`（2026-09-18 发，用户指令「发一个新的小版本」＋在岔口上选定「版本号改成 0.9.0（新功能按语义化版本该进 minor 位）」；**主题是「写作模式变成真的 Typora 形态」**）：① **写作模式的块级渲染第一次进发行版**（PR #60 那条线，43 个文件 / 约 10845 行）—— 非光标块整块显示成引擎切片、光标所在块展开源码，点切片落到精确字符、可拖选跨块复制、`#link` 是可点热区；② **PR #60 审查的四条严重项 + 一批一般项**（都写成了下方「写作模式的块级渲染」那节里的"勿回退"红线：`blocks` 键永远要发、块级结果要确认"请求时那份文档 == 当前文档"、`HIT_CACHE` 带几何编号、合成期不重建装饰、Shift+点击=扩选、公式字号 = 正文字号、链接热区 `mousedown` 要 `preventDefault` …）；③ 另有两笔是本会话的 UI 改动 —— **所有弹出来的面板统一白底黑字、浮层条目浅灰**（用户要求「把所有弹出来的窗口，和 错误 警告 的浮窗（把每一个条目改成灰色），改成白色」）、**删掉「保存空文档」确认窗**（用户发来截图后要求「删了这个窗口」）。**这一版数字：`wysiwyg` 290 / `writing-blocks` 133 / `writing-blocks-visual` 76 / `writing-blocks-hit` 136 次点击（27 项）/ `writing-mode-scenes` 64 / `wysiwyg-visual` 20 / 前端单测 707 项 / Rust 60 项**。**发版决策记一笔**：用户先说「y 往后一个」（= 0.8.5），我把"main 上已经并了块级渲染（10k 行新功能）"这个岔口摆出来问了一句，他改成 **0.9.0** —— 以后再遇到"补丁位里躺着新功能"，就照这次的办法：**先说清楚这一版会带什么，再让他定数字**。
- 版本 `0.8.5`（**未发布** —— 版本号一度改成 0.8.5 又按用户选择改成 0.9.0，没有这个 tag）
- 版本 `0.8.4`（2026-09-18 发，用户指令「发新小版本」；**主题是「诊断浮层能复制 + 菜单变白 + 警告浮层对齐错误浮层」**）：① **诊断信息可复制**（用户要求「需要功能：复制错误信息」）—— 错误/警告浮层**每条右侧**「复制」、**标题行右侧**「复制全部」，复制文本 = `<路径>: 行 N, 列 M：<消息>`（**路径贴行列前面是用户当场指定的**：include/import 用文件自己的绝对路径、主源回退当前文档路径、未保存文档省略前缀），「复制全部」首行是浮层标题原文；写剪贴板走新模块 `clipboard.ts`（`execCommand` 主路径 + `navigator.clipboard` 兜底），浮层文字同时放开拖选（状态栏整条是 `user-select: none`）。② **警告徽标的浮层交互对齐错误徽标**（用户反馈「点击警告 / 关闭警告的行为应该和错误是一样的」）—— 补上 Esc 关闭、点浮层外部关闭、点定位条目跳转即关、打开瞬间视口收边，并顺带修掉「错误修好后留一个只有标题的空浮层」「警告重新出现时浮层自己弹开」两处状态不同步；两个徽标现在共用一份 `openBadgePopover` 状态、行为只写一遍。③ **顶部菜单栏的展开菜单改成白底黑字**（用户要求「把上方菜单栏的展开菜单改成白色」）—— 面板固定浅色、**深色主题下也一样**，悬停从「亮蓝底 + 白字」换成「浅蓝底 + 蓝字」（白底上旧组合对比度不够）。细节分别见「两套 UI / 状态栏最左是错误与警告计数」与「菜单不夺焦」两条注解。**这一版数字：验收 270 项 / 前端单测 505 项 / Rust 44 项**（Rust 未改）。另：这一轮还修了验收脚本自己的一个状态泄漏（`gotoSim` 的"清完立刻导航"会让新页带着上一段的缩放起步，详见「测试」节末），以及把「诊断可复制」这条链路的 UI 用假剪贴板锁进验收（桩里包了一层 `document.execCommand`）。
- 版本 `0.8.3`（2026-09-18 发，用户指令「v0.8.3」；**主题是「`#import` 别的文件夹里的文件」**）：① **项目根跟着文档的引用走**（细节见「编译数据流 / 文件语义」）—— `#import "../touying/z.typ": *` 这类**文档目录之外**的引用以前一律报 `would escape the project root`（typst 的 `..` 按虚拟路径判越界，根钉死在文档目录就等于"只能导入同目录与子目录"）；现在根按文档实际引用到的相对路径自动放宽，**按 `..` 层数**算最低可满足根并取其公共祖先（别写成"目标文件的公共祖先"，`Typst/touying/head.typ` 是反例），跨卷/跨盘仍是硬限制但诊断里会写明当前项目根。**用户那份幻灯片端到端实测 `ok=true` / 13 页**（含 `../touying/z.typ` 与模板内部的 `@preview/touying:0.7.3` 等）。② **顺带修掉从 0.4.0 起的老 bug：桌面版编译错误一条红波浪线都不画** —— Rust 对主源发 `"path":null` 而前端只认"键缺失/空串"（错误计数与错误列表都正常，所以看着像能定位）；现在契约只留一种表示 + 桩按真实形状发 `path: null` + 验收第 42 组锁死。**这一版数字：验收 243 项 / 前端单测 498 项 / Rust 44 项。** 上一轮（**0.8.2**，2026-09-18 发，用户指令「发 0.8.2」；**主题是「脚本错误 + 打不进字，只能重启」**）：① **语法高亮换到上游无 wasm 的 Lezer 解析器** —— `codemirror-lang-typst` 0.4.0 → **0.6.0**、`typst()`（wasm-bindgen + `typst_syntax_bg.wasm`）→ **`typst_lezer()`**（`codemirror-lang-typst/lezer`）。那个 wasm 解析器被「文档变更时 `edit()`」与「解析时 `tree()`」两条路共用，重叠即撞借用守卫 panic，**panic 发生在 CodeMirror 的事务里 ⇒ 打字被丢掉、且守卫不复原 ⇒ 此后每次输入都失败、只能重启**（用户原话「出现 BUG 还会导致无法输入，需要重启」）；前端现在**一个 wasm 都没有**（生产构建产物已无 `.wasm`）。② **预览永不横滚**（画布恒 ≤ 栏宽，文档自带 `#set page(...)` 也一样）。上一轮（**0.8.1**，2026-09-17 发，用户指令「v0.8.1」；主题是「Ctrl+滚轮到上限就不动了」）：① **修掉滚轮死区** —— 一次滚轮的位移可以不足一档（高倍缩放下每格位移变小），而档位 10% 一格、`clampZoom` 会把折算出来的 4% 圆整抹掉，每个事件又独立计算 ⇒ 那种滚轮**永远**动不了，状态栏还在谎报「到边界了」（**在 100% 也这么说**）；现在 `accumulateWheelSteps` 把余量跨事件攒着、不足一档时改口说「攒到 N%」，验收第 40 组用真实 40px 滚轮锁住（详见「界面缩放」那节最后一条注解）。上一轮（**0.8.0**，2026-09-16 发，用户指令「就发 v0.8.0」；主题是「缩放只归用户，软件别自己动」）：① **撤掉整套「复核发现引擎没动就把档位拉回去」的机制** —— 用户第六轮反馈「用 Ctrl + 滚轮 会回退」「往下滚没反应」，根因是我们在**判错**（他那台机器上 CSS 布局宽度与 dpr 两条判据都读不出缩放）之后，把用户要的档位改回引擎（自认为）给的值，所以每次缩放都被自己弹回；现在 `setZoom` 只在用户操作时调一次，之后 `observeZoomEffect` **只观察、绝不改状态**（代价：引擎上限那类机器上死区会回来，用户明确接受）。② **`Ctrl+Shift+=` / `Ctrl+Shift+-` 键盘调档 ±1 格**（不经过滚轮手势，既是可用操作，也是判别「滚轮事件到底有没有到页面」的判据）。③ **`Ctrl+N` 新建前确认 + 空文档覆盖已有文件前确认**（用户问「编辑器会清空文件吗？？」后补的两道防护；**后半条 2026-09-18 已被用户要求删掉**（原话「删了这个窗口」）—— 现在空文档存进已有文件是直接写空、不再问，见「写盘只有一条路」那条注解）。④ **新应用图标**「叠纸 + T」。回看：0.7.10 是「把看着能用、其实没生效的问题修掉」那一版（`.ttc` 字体集合逐 face 注册 / 缩放基准被自己的 resize 带偏 / 回车缩进 / Tab 四格 / 关于弹窗重写）；**0.7.4 是「自动更新真的能生效」的第一个版本**（0.7.3 引入 updater 但仓库当时私有，客户端拉不到清单；仓库公开 + 0.7.4 发布之后这条链路才第一次跑通）；**0.7.8 起启动就检查更新、点过「稍后」不再自动打扰**。`main` 与 `origin/main` 同步；每次打 tag 后 `release.yml` 建草稿 Release，**由后台一次性任务自动 Publish**（不必等、不必问）。**发版后的验收就照这两条 curl 做**（见下）。
- **发行状态（2026-09-18 更新）：`v0.9.0` 已发布并匿名验证过（2026-09-18 22:17）**：`latest.json` 200 / `version: 0.9.0` / 安装包 200（**16749934 字节**，`Typst-pad_0.9.0_x64-setup.exe`）/ **验签通过**（keyid `4e79c0dd71347bb5`、alg `ED`）；`isDraft` 为 `false`、资产齐全（latest.json / exe / exe.sig / msi / msi.sig —— 五个资产 14:17:12 齐、14:17:23 Publish）。流程同前几版：tag 推上去后由后台一次性任务盯草稿 → 资产齐了直接 Publish → 立即匿名验收（**第 1 次就退出码 0**，CDN 这次不用等），**全程不阻塞回合**；复验：`node scripts/verify-release.mjs 0.9.0`。安装包比 0.8.4 大了 **约 46KB**（16703901 → 16749934）—— 块级渲染那条线的 Rust 代码（`block_geometry.rs` + 命中测试 + 几何编号）就这么多。上一版 **`v0.8.4`（2026-09-18）**：安装包 **16703901 字节** / 验签通过（同 keyid），主题是「诊断浮层能复制 + 菜单变白 + 警告浮层对齐错误浮层」。上一版 **`v0.8.3`（2026-09-18）**：安装包 **16702123 字节** / 验签通过（同 keyid）—— 那一版的主题是「`#import` 别的文件夹里的文件」，并顺带修好「桌面版编译错误一条红波浪线都不画」这个从 0.4.0 起的老 bug。上一版 **`v0.8.2`（2026-09-18）**：`latest.json` 200 / `version: 0.8.2` / 安装包 **16698870 字节** / 验签通过（同 keyid）—— **它比 0.8.1 小了 ~10 万字节，那正是被剥掉的 wasm**（`typst_syntax_bg.wasm`）。再上一版 **`v0.8.1`（2026-09-17）**：安装包 **16800426 字节** / 验签通过（同 keyid）。**`v0.8.0`（2026-09-16）**：安装包 **16806820 字节** / 验签通过；**一条待查的观察（不紧急）**：`gh run list` 里 v0.8.2 那次 Release run 记的是 `failure`，但资产齐全、签名也验过 —— 疑似上传/发布之后的收尾步骤（缓存之类）失败，不影响发布结果，根因还没查。旧版本清单：`v0.8.1`、`v0.8.0`、`v0.7.10`、`v0.7.9`、`v0.7.8`、`v0.7.7`、`v0.7.6`、`v0.7.5`、`v0.7.4`、`v0.7.3`、`v0.7.2`、`v0.7.0` 已发布；只有 `v0.7.1` 还是草稿**（handover 里曾把 v0.7.2 误记成草稿）。**0.7.10 已匿名验证过（2026-09-15）**：`latest.json` 200 / `version: 0.7.10` / 安装包 200（16772701 字节，`Typst-pad_0.7.10_x64-setup.exe`）/ **签名与配置里的公钥验签通过**（keyid `4e79c0dd71347bb5`、alg `ED`）；`gh release view` 的 `isDraft` 为 `false`，资产齐全（latest.json / exe / exe.sig / msi / msi.sig）。0.7.9 那次的记录：安装包 200（16762871 字节）/ **签名验签也通过**（keyid `4e79c0dd71347bb5` + ed25519 verify：公钥取 `plugins.updater.pubkey` 第二行，签名的 alg `ED` = 先 BLAKE2b-512 再签，node:crypto 就能验）。发版时 `release.yml` 建的仍是**草稿**，**必须手动 Publish**（或按下方约定直接发）——草稿资产不对外，客户端拉不到 `latest.json`，自动更新不会生效。
...
CDP_PORT=9335 BROWSER_CHECK_PORT=1425 node scripts/browser-check/writing-blocks.mjs        # 交互（桩产物，133 项）
...
CDP_PORT=9335 BROWSER_CHECK_PORT=1425 node scripts/browser-check/writing-blocks.mjs  # 块级渲染交互验收（桩产物，133 项：切片/展开/窗口化/竖直移动（代码模式语义）/点击锚定/翻页/编译失败/块内 Enter/各种输入/拖选复制/整块选中/打包字体/公式字号/输入法合成/Shift 扩选）
npm run fixtures:blocks && CDP_PORT=9335 BROWSER_CHECK_PORT=1425 node scripts/browser-check/writing-blocks-visual.mjs  # 块级切片几何等价 + 链接热区 + 点热区后焦点（真实产物，76 项）
...
src/lib/typst-highlight.ts  # 压掉**编辑器默认高亮**（@codemirror/language 的 defaultHighlightStyle）给标题加的下划线（见「写作模式」那节；根因 2026-09-18 才复核准）
...
src/lib/doc-utils.ts        # 文档纯函数：isEffectiveDirty（空文档视为未修改）、ensureTrailingNewline（前缀末行补换行）（`needsBlankOverwriteConfirm` 2026-09-18 随「保存空文档」确认窗一起删除）
...
src/lib/preview-scale.ts    # 预览缩放纯逻辑：**按栏宽重排**（栏宽→页宽 pt）+ 等比缩放回退路径（文档自带纸型时）——**两条路的画布都恒 ≤ 栏宽 ⇒ 预览永不横滚**（2026-09-18 起，见「界面缩放」那节）（可单测）
...
### 写作模式的块级渲染（"渲染表面 + 源码透镜"，**0.9.0 起已发布**）
...
+ `#import` 项目根 + 诊断浮层可复制；这条块级渲染线 2026-09-18 通过 **PR #60** 合并进 `main`
（merge commit `071cd1d`），随后四笔审查修复直接推上 main（`9fab355`/`1afe7d1`/`6d9b551`/`dd7b191`），
**`v0.9.0` 是它第一次进发行版** —— 所以这一版的更新说明（`CHANGELOG.md` 的 `[0.9.0]` 段，
也是客户端更新弹窗里显示的那份）把功能与那四笔修复都写全了。）
...
- **编辑期间必须让块表"跟着走"（红线，用户报过「在一块内 Enter 插入块的时候会有问题」）**：
  ...
  - **Shift+点击 / Shift+拖选切片 = 扩选**（PR #60 审查第 6 条）：`CropSelection.commit()` 原本一律
    `EditorSelection.single(anchor, head)`，而锚点是"按下那一刻解析出来的位置" ⇒ 扩选反而**把原选区
    收掉**。现在与 CM 默认同口径：固定端（`extendFrom`）= 原选区（空选区时是光标）。
    验收 `writing-blocks.mjs` 第 20 组 5 项；**去掉修复时红 3 条**（实测 `{"empty":true}`）。
  - **链接热区的 `mousedown` 必须 `preventDefault`**（PR #60 审查第 6 条）：只 `stopPropagation`
    的话浏览器会把焦点给这个 `<a>`，编辑区失焦 ⇒ 用户点完链接回来**一个字都打不进去**。
    验收在 `writing-blocks-visual.mjs`，**判据必须是"焦点就在 `.cm-content` 上"** —— 只判"焦点在
    编辑区里面"是恒真的（热区 `<a>` 本身就是 `.cm-content` 的后代，实测关掉 `preventDefault`
    它照样绿）。
  - 阶段 3 的验收：`writing-blocks.mjs` **第 14 组**（拖选跨块 + Ctrl+C 复制 + 拖选后打字替换选区）、
    `writing-blocks-visual.mjs` 的**链接热区**四条（位置与真实几何一致 ≤2%、点热区 → opener 收到 URL、
    点热区不动光标、**点热区后焦点仍在编辑内容元素上**）、`live-preview.test.ts` 的 jsdom 用例（拖选把选区落到源码区间、链接热区按百分比定位）、
...
- **公式字号 = 正文字号，跟着模式走（PR #60 审查第 2 条）**：`MATH_SIZE_PT = 12` 是"写作模式正文
  16px"那个年代的常数，而本线让写作模式正文跟着文档走（默认 11pt = 14.6667px）—— 写死 12pt 会让
  光标所在块里的公式比周围正文大 9%、也比同一公式在切片里的样子大。现在由父组件按模式给：
  写作模式 = 文档 `textPt`（`Editor.svelte` 的 `mathSizePt`），源码模式兜底 `MATH_TEXT_PT = 10.5`
...
- **打字期间不编译（红线，用户报「输入手感很差」）**：写作模式下**每一次按键**都会走一遍
  ...
  - **`writeCompileTimer` 跑完必须置回 `undefined`**（PR #60 审查第 8 条）：它同时是"有没有挂着的
    块编译"的判据（`handleMathRequest` 据此把公式优先级提前）。不置回的话，**每个公式请求都会在
    240ms 后再排一次整篇编译** —— 公式多的文档接近双倍编译量。
  - **`compile_blocks` 的 `blocks` 键永远要发**（哪怕失败时是空数组，PR #60 审查第 1 条）：
    前端用"`blocks` 是不是数组"区分「后端没实现这个命令」（旧安装包 / 桩）与「这一次编译失败」。
    它原先是 `skip_serializing_if = "Vec::is_empty"`，而 `fail()` 正是把它置空 ⇒ 失败 JSON 里没有
    这个键 ⇒ **真机上任何 typst 错误都被读成"后端不支持"**：退回整页预览、切片不撤、错误块不展开，
    `applyBlocksResult` 的失败分支成了死代码。前端那个"形状不对就 unavailable"的判据现在**只看
    `ok` 键**；桩**故意不发** `blocks`（真 Rust 侧早先的形状）来锁这条路径。Rust 侧单测
    `failure_output_always_carries_blocks_key`。**与 `path:null` 是同一类陷阱：桩按契约写、真后端不这么发。**
  - **块级结果回来时必须确认"请求时的那份文档 == 当前文档"**（PR #60 审查第 3 条）：
    写作模式的编译是去抖的，这期间 `compileSeq` 不会自增 ⇒ 旧结果会被套到新文档上，**字节偏移错位**
    （旧切片盖住被移动的正文 / 同一段既是切片又露成源码），编译失败时还不自愈。
    `remapBlocksForEdit` 救不了它（比较的是同一份新文档，前后缀差分看不出差别）。`runCompile` 里记
    `requestDoc`，不一致就丢弃（编辑那条路已经排了一次去抖编译，会带正确坐标回来）。
  - **输入法合成期间不许重建装饰**（PR #60 审查第 5 条）：`blocksVersion++` 在每次编辑都发生，
    而 Editor 里那个"重整装饰"的 `$effect` 原本没有 `view.composing` 守卫 —— 合成中途换掉 widget DOM
    会把候选串与合成状态一起弄坏（中文用户每天都要走这条路）。现在合成期间攒着、
    `compositionend` 后补一次刷新；验收第 19 组用 CDP 的 `Input.imeSetComposition` 走真实合成路径。
...
- **点击定位（阶段 2）**：点切片 → 光标落到**点到的那个字符**。链路 =
  ...
  **绝不吞掉点击**。三道闸门：块表必须与当前文档一致（`writingBlocksDoc === doc`）、是
  **精确**的（`writingBlocksExact`，编译失败后沿用旧切片时区间是估算的）、
  **几何编号对得上**（`writingGeometryId`，见下）。
  - **`HIT_CACHE` 是进程级的，必须带几何编号**（PR #60 审查第 4 条）：应用支持多窗口
    （`Ctrl+Shift+N`），窗口 A 编译完、窗口 B 再编译一次之后，A 里点击就会拿 **B 的排版几何**去找
    最近字形 —— 结果被钳进 A 的块区间，**点错字而不报错**；前端那两道闸门都是**每窗口**的，
    看不见对方。现在 `compile_blocks` 返回自增的 `geometryId`，`block_hit_test` 带回来比对，
    对不上 → `None`（前端退回"光标落到块首"）；不传编号（旧前端 / 内部探针）仍按老行为。
    Rust 单测 `hit_test_refuses_geometry_owned_by_another_compile` 锁三条分支；
    `writing-blocks-hit.mjs` 那 136 次点击现在真的走这条校验（编号错就全红）。
...
- **两套 UI（仿 Typora）**：`viewMode: "write" | "source"`（取代旧的 `livePreview` 布尔，旧存档自动迁移）。
  - **写作模式**（默认，单栏）：灰底 + 居中纸张（`.panes.single .editor-pane .pane-body` 上用 `--bg-backdrop`/`--bg-paper`）、衬线正文（Noto Serif CJK SC，与预览/PDF 输出同字体）、16px/行距 1.9、**隐藏行号槽**与当前行高亮（都在 `Editor.svelte` 的 `.editor-host.write` 样式里）、状态栏显示「写作」且不显示行列。公式/标记就地排版。
    ...
    - **标题正文没有下划线**（2026-09-14 用户反馈「`== 1` 在写作模式有下划线」；**2026-09-18 复核纠正过根因**：那条下划线不是 typst 包自带的，而是 **`@codemirror/language` 的 `defaultHighlightStyle`**（`basicSetup` 的兜底高亮：`{tag: tags.heading, textDecoration: "underline", fontWeight: "bold"}`），typst 只是把整棵 Heading 子树标成 `tags.heading`（`"Heading/..."`）而踩上了它。0.6.0 的 typst 包里已经没有 heading 规则，下划线照旧 —— 换解析器时我一度把补丁删了，浏览器验收当场把它揪回来）。修法不变：`typst-highlight.ts` 再挂一份 `textDecoration: "none !important"` 压掉它 —— 两者是**同一个元素上的两个类**（CM6 的高亮是「类并集」，见 `syntaxHighlighting` 文档原话），同优先级下谁赢只看样式表顺序，而 StyleModule 的挂载顺序由扩展树决定，**所以 `!important` 不是可有可无的**（单测里有一条对照用例专门证明这一点）。
  ...
  - **源代码模式**（`Ctrl+E`，双栏）：等宽 14px + 行号 + oneDark（暗），右侧整页预览；此模式下 live-preview 整体关闭（看到的是真正的 Typst 源码）。
    - **键位归属：`Ctrl+/` = 注释、`Ctrl+E` = 切换模式**（2026-09-18 用户要求「`Ctrl+/` 归注释、模式切换改 `Ctrl+E`」）。为什么必须分开：两件事曾经**同时**挂在 `Ctrl+/` 上 —— 编辑器的 CM keymap 处理 `Mod-/`（`editor-keymap.ts`）时只 `preventDefault()`、**不阻断冒泡**，而菜单的 window 级匹配（`menu-keys.ts` 的 `findShortcutTarget`）**不看 `defaultPrevented`** ⇒ 按一次既注释又切模式。现在 `Ctrl+/` 只归注释（VS Code 习惯），模式切换在「视图 → 源代码模式」的 `shortcut: "Ctrl+E"` 上（那行文本就是真正生效的绑定）。`Ctrl+E` 经核对完全空闲：src 内无占用、CM `basicSetup` 的 keymap 无 `Mod-e`、引擎自带加速键已被 `lib.rs` 的 `DisableBrowserAccelerators` 关掉。**验收第 20 组新增 3 条**锁住它：`Ctrl+/` 后文档里出现 `//` 且状态栏仍是「写作」、`Ctrl+E` 切到源码、再按一次回写作。
    - **注释符号不用我们自己补**（曾经的"待办"已作废）：`codemirror-lang-typst` **0.4.0**（wasm 那代）的语言数据只声明 `block: { open: "/*", close: "*/" }`，所以那时 `Ctrl+/` 打的是 `/* … */`；**0.6.0 两个入口都声明了 `line: "//"`**（`dist/lezer.js` 与 `dist/index.js` 的 `typstLanguageDataConfig`），`toggleComment` 于是直接用行注释 —— 不必再挂一份 `languageData`（真挂也要 `Prec.highest`：`@codemirror/commands` 的 `getConfig` 取 `data[0]`，同优先级会输）。`editor-keymap.test.ts` 里那条 `// aaa` 用的是**注入的替身 languageData**，真实路径由验收第 20 组在浏览器里锁。
  ...
  - **界面缩放（Ctrl+滚轮 / `Ctrl+Shift+=`·`Ctrl+Shift+-`）**：整个界面等比放大/缩小（**不是**分栏宽度——用户反馈「字太小看不清」要的是字变大；分栏宽度那套手势已移除）。范围 **50%~250%**、一次一格 10%，默认 100%。纯逻辑在 `zoom.ts`（可单测），页面侧是 `handleZoomWheel`/`setUiZoom`/`applyUiZoom`，**走 Tauri 的 webview 缩放 `setZoom`**（权限 `core:webview:allow-set-webview-zoom`）。
    ...
    - **~~引擎不接受的档位不许记进状态~~（2026-09-14 的旧政策，已被 2026-09-16「缩放只由用户改」取代 —— 下面这段留作历史与成因记录）**（2026-09-14 **四轮**实机反馈「放大根本没用，缩小有用」→「最大后无法用滚轮缩小」→「缩放到最大后无法从 Ctrl+滚轮缩小」→（直接给状态栏截图）「引擎把 150% 限制在 100%」都是**同一个机制**：引擎没接受放大，而 `uiZoom` 照旧涨到上限 250%，于是从 250% 往下滚要滚十几档才有反应，看着就是「缩不了」）。`verifyZoomApplied` 的做法：① 首次应用缩放前 `ensureZoomCalibration` 先设 100%（排掉 WebView2 记住上次站点缩放的干扰）并记下**此时的 CSS 布局宽度**当基准；② 每次调档后按 **`100% 基准宽度 ÷ 当前布局宽度`** 反推**引擎实际接受的档位**（见 `zoom.ts` 的 `zoomFromWidths`）；③ 与请求值不一致就把 `uiZoom` **拉回引擎给的档位**，状态栏写明「界面缩放未生效：引擎把 150% 限制在 100%（量了 4 次；布局宽度没变（1379px）；dpr 1.50）」。放大被拒时档位原地不动、界面与状态一致，缩小立刻有效；`uiZoom` 被赋值会再触发一次 `$effect`，但那时 `applied == target` 会直接返回，不会循环。
      ...
      - **预览永不横滚（两条路的画布都夹在栏宽内）**（2026-09-18，用户反馈「为什么预览框还是会出现下方的滑动条」后选定「A. 永不横滚」）：预览页宽是编译期决定的 —— ① 文档没写纸张 → 我们注入的 `#set page(width:)` **按栏宽重排**（0.7.9 起）；② 文档**自己**写了 `#set page(...)`（`paper:`/`width:`/`height:` 都算）→ 注入被覆盖、退回「固定版心 + 等比缩放」老路。老路上界面缩放会把 **CSS 视口一起缩小**（1400px 窗口在 150% 下只有 933 CSS px、预览栏 451px），而画布被自然尺寸封顶在 A4 ≈ 568px ⇒ **画布比栏宽还宽** ⇒ 底部横条（浏览器验收实测：150% 溢出 117px、250% 溢出 304px），且画布在 568px 就不再变大 —— 后期这条横条毫无收益。现在 `previewScale` 取 **`min(缩放前栏宽, 当前栏宽, 自然尺寸)`** ⇒ 画布恒 ≤ 栏宽；代价是固定版心的文档「铺满预览栏」之后不再继续放大（用户知情并选定）。**第 41 组**用真机几何（Emulation 压视口 + 键盘设 uiZoom）把 150%/250% 两条都锁上；第 30 组那条「退回等比缩放」的判据改成「**产物页宽仍是 A4**」（旧指纹在新规则下失效）。
    ...
    - **状态栏最左是「错误 / 警告」计数**（用户要求：「加入警告，用类似 vscode 的图标（三角形内部有感叹号）…另外，把这两个东西都移到最左边」，随后又给了参照图 **⊗ 0 ⚠ 0** 定序）：最左是一个 `.badge-group`，**错误在左、警告在右**（与 VS Code 状态栏同序），两者都**常驻显示**（无问题时是 0，与 VS Code 状态栏一致；常驻还能避免数字出现/消失时整条状态栏左右抖动），状态文字排在其后。
      ...
      - **两个徽标的 Popover 必须成套一致（2026-09-18 修，勿只改一半）**：用户反馈「状态栏 点击警告 关闭警告的行为应该和错误是一样的」—— 当时警告侧只有「点一下开合」和错误侧同款，**少了四条**：Esc 关闭、点浮层外部关闭、点条目跳转后收起、打开瞬间的视口收边（`cursor: pointer` 倒是早就有了：警告徽标的类名里带着 `error-badge`，`.error-badge.clickable` 那条规则对它同样生效 —— 所以别再"顺手"把这个类名从警告徽标上摘掉）。修法是把两份状态合成**一份** `openBadgePopover: "none" | "errors" | "warnings"` （+ 两个 `$derived`：`errorPopoverOpen` / `warningPopoverOpen` = 开着 **且**有内容），浮层行为（内容空了收起 / Esc 与点外部 / 打开瞬间收边 / 共用的 `popoverStyle()`）**只写一遍**；同一时刻只开一个浮层（编译器失败时前端会把 `compileWarnings` 清空，所以两个列表本来也不会同时非空）。两个 `$derived` 顺带修掉两个"状态与 DOM 不同步"的老毛病：**错误列表清空后以前会留一个只有标题的空浮层**、**警告列表消失后 `showWarnings` 仍是 true ⇒ 同一份文档里警告一回来浮层自己就弹开**。列表条目点击改名 `onDiagnosticItemClick`（两个列表共用，两种分支都顺手收起浮层）。验收**第 43 组（14 项）**锁死：警告侧 9 条（可点光标 / 开 / 合 / Esc / 点内部不关 / 点条目跳转即关 / 点外部关 / 消失后不残留 / 不自己弹回来）+ 错误侧 4 条对照（含"点条目跳转即关"、"消失后不留空浮层"）+ 400px 窄视口下警告浮层被收进视口内（该宽度下不修必红：实测右缘 **406 > 400**）。**这一轮改动尚未发版**（等用户说版本号；CHANGELOG 与三处版本号未动）；0.8.3 那一版验收是 243 项，本轮加到 257 项。
      - **诊断信息可复制（2026-09-18，用户要求「需要功能：复制错误信息」）**：两个浮层每条右侧一个「复制」按钮、标题行右侧一个「复制全部」，复制文本 = `<路径>: 行 N, 列 M：<消息>`；「复制全部」首行是浮层标题原文（`编译错误（2 处）`）其后每条一行、末尾不带换行。**路径来源**：诊断自带路径优先（include/import 的文件在 Rust `path_of` 里已 realize 成绝对磁盘路径，包内文件是 `@preview/...` 虚拟路径），主源回退到当前文档路径（`filePath`），未保存文档整段省略路径前缀 —— 为此 `LocatedErrorItem` 新增了 `path` 字段（`buildErrorListItems` 以前把它丢了，`warningItems()` 也一并补上）。格式化全在 `error-list.ts` 的两个纯函数（`formatDiagnosticForClipboard` / `formatDiagnosticListForClipboard`，有单测）；消息一律用**界面上展示的那份**（警告是 `describeCompileWarning` 翻过的中文），不是引擎原文。写剪贴板走新模块 `clipboard.ts` 的 `copyPlainText`：临时 textarea + `document.execCommand("copy")` 为主、`navigator.clipboard.writeText` 兜底（与 `Editor.svelte` 里 cut/copy 的取舍同源：execCommand 同步、在点击手势内、不需要权限），复制前后保存/恢复 `activeElement`，不留焦点副作用。两条**别再回退**的结构约束：① 浮层要有 `user-select: text`（状态栏整条是 `user-select: none`，不放开就拖选不了）；② 一条诊断 = `.error-item-row` 里的「条目按钮 + 复制按钮」两个**兄弟**按钮 —— **按钮不能嵌按钮**（HTML 非法），所以 row 这层不能省，`.error-item` 在 row 里要 `flex: 1 1 auto; min-width: 0`（少了 min-width 长消息会把「复制」挤出浮层）。复制**不关浮层、不跳转、不动光标**（按钮在 wrap 内，也不会触发"点外部收起"）。验收**第 44 组（9 项）**锁：两个浮层的按钮齐全、复制到的字符串逐字节相等（错误侧 + 警告侧中文提示）、复制后浮层仍开着、状态栏出现「已复制」、点「复制」不会顺带跳转（光标仍在原行列）、`user-select: text`。**桩里加了"假剪贴板"**（`browser-dev-stub.ts` 包一层 `document.execCommand`，把被选中的文本记进 `window.__browserDevCopied` 并当成功）——否则无头环境里"复制到的到底是什么"无从断言；**真机（WebView2）上真实落进系统剪贴板这一步只能由用户点一下确认**（无头验收覆盖不到引擎剪贴板）。**本轮改动尚未发版**；验收计数从 257 → 266（复制 9 项）→ **270 项**（菜单白底 4 项）。
...
- **所有「弹出来的面板」固定白底黑字、浮层里的条目是浅灰（用户 2026-09-18 两条要求，第二条是「把所有弹出来的窗口，和 错误 警告 的浮窗（把每一个条目改成灰色），改成白色」）**：色值**只有一份定义**，在 `+page.svelte` 的 `:root` 里那组 `--panel-*`（`--panel-bg: #ffffff` / `--panel-soft-bg: #f0f0f0`（浮层条目、输入框、按钮、进度槽）/ `--panel-border: #d9d9d9` / `--panel-fg: #1f1f1f` / `--panel-fg-dim: #6b6b6b` / `--panel-accent: #0b6bb5` / `--panel-hover-bg: #e8f2f9` / `--panel-hover-fg: #0b6bb5` / `--panel-shadow`），**四处共用**：菜单下拉（`MenuBar.svelte` 的 `.menu-dropdown`，第一版只改了它，当时就地写死了五个色值，现已改成引用 `--panel-*`）、右键菜单（`ContextMenu.svelte` 的 `.context-menu`）、四个弹窗（`.modal` = 关于 / 设置 / 更新 / 未保存确认）、错误与警告浮层（`.error-popover`）。**不跟应用主题走** —— 深色主题下也是白底黑字（用户提这两条需求时用的都是深色主题）。
  - **用法：在面板根元素上把主题变量就地重绑一遍**（`--fg` / `--fg-dim` / `--bg-pane` / `--border` / `--accent` = 对应的 `--panel-*`），子元素（标题 / 正文 / 按钮 / 输入框 / 更新说明 / 菜单项 / 诊断条目 / 行列 / 复制按钮）本来就只用这几个变量，重绑一次就整体变浅色，**不用去数有几个标题几个按钮**。**必须同时给面板自己的 `color: var(--panel-fg)`**：不写这条，继承下来的是 `.app` 上算好的 `#d4d4d4`（深色主题的浅灰），白底上等于看不见 —— 与「菜单不夺焦」那条注解里"白底 + 浅灰字 = 看不见"是同一个坑。
  - **别再改回 `var(--bg-toolbar)` / `var(--fg)`**：那两条在暗色主题下给的是深底浅字（`--fg` 是 `#d4d4d4`），浅色主题下 `--bg-toolbar` 是 `#ececec` 的**灰**面板 —— 正是用户报的那种"灰窗口"。悬停一律"浅蓝底 + 蓝字"（白底上用旧的"亮蓝底 + 白字"对比度不够）；面板变白 ⇒ 阴影用 `--panel-shadow`（`rgba(0, 0, 0, 0.2)`），否则白块边缘发黑。
  - **条目灰色只用在浮层里**：`.error-item` 的底色走 `--bg-pane`，在 `.error-popover` 里已重绑成 `--panel-soft-bg`；白面板 + 灰条目是用户指定的组合（前一版是**反过来**的"浅色主题下灰面板 + 白条目"）。不可点击的 `.error-item-generic` 悬停时底色也不许变（要显式回落，否则会跟着 `.error-item:hover` 变蓝）。
  - 验收：第 16 组 4 项（浅色 / 深色主题下各断言菜单面板 `rgb(255, 255, 255)`、文字 `rgb(31, 31, 31)`）+ **第 45 组 11 项**（**先把主题切到深色**再量：设置弹窗 / 关于弹窗 / 警告浮层 / 错误浮层 / 右键菜单 —— 面板白、正文 `rgb(31, 31, 31)`、灰字 `rgb(107, 107, 107)`、浮层条目 `rgb(240, 240, 240)`、多行输入框浅灰底深色字、右键菜单**走真实右键**（`Input.dispatchMouseEvent` 的 `button: "right"`）且禁用条目照旧灰字）。**改回主题变量这一组立刻红**；浅色主题下改回去只会变成 `#ececec` 的白，肉眼不容易发现。这一组顺带补上了右键菜单一直没有的浏览器验收。
...
- **持久化**：`viewMode`（+ `showPreview` + `editorWrap`）与主题一起存 localStorage；旧的 `livePreview` 布尔自动迁移为 `viewMode`；视图菜单（`Ctrl+E`）切换。
...
- **文件语义**：主文档源码由前端传入（未保存也可编译）；**项目根 = 文档所在目录，再按文档实际引用到的相对路径自动往上放宽**（`resolve_project_root` + `collect_required_roots`），相对 `#import`/`#include` 从磁盘按 typst 语义解析（相对路径基于引用文件所在目录）；`document_path = None`（未保存）时 `check_relative_imports` 预检 `#import`/`#include`，给出"需要先保存文档"的明确诊断。
  - **为什么要放宽（2026-09-18 用户报「还是没法 `#import` 别的文件」，场景是 `Typst/2026.6.3-随机化和近似算法/*.typ` 里写 `#import "../touying/z.typ": *`，报 `path "../touying/z.typ" would escape the project root`）**：typst 的 `..` 是按**虚拟路径**判越界的（`typst-syntax` 的 `Segments::push_component`：`..` 弹不动就 `PathError::Escapes`），目标文件明明躺在磁盘上、只是比根高一层也照样报错 —— 根钉死在文档目录就等于"只能导入同目录与子目录的文件"。
  - **放宽规则照引擎语义算，别写成"目标文件的公共祖先"**：某文件（目录 `D`）里一条相对路径，按 `Normal:+1 / ..:-1` 折算出冲出 `D` 的层数 `need`，根至少要是 `D` 的第 `need` 层祖先；最终根 = 所有引用（含**被引用文件自己的引用**，递归扫描 `ROOT_SCAN_MAX_DEPTH=8` + `visited` 去重）的最低要求的公共祖先。反例（真实存在）：`Typst/touying/head.typ` 里那句 `#import "../touying/z.typ"` 按目标算会得到 `Typst/touying`、仍然越界；单测 `project_root_widening_rules` 与 `relative_import_parent_dir_ok` 钉住这两层。
  - 跳过 `@`（包，自己的虚拟根）、`/` 开头（根相对，永不越界）与绝对路径（typst 里本来就非法）；目标是 `.typ` 且磁盘上存在时递归看它引用了什么。目标不存在也按**词法**放宽（`lexical_normalize`，不碰文件系统），于是"名字写错"报的是 `file not found` 而不是越界（`relative_import_missing_target_reports_not_found`）。
  - **跨卷/跨盘仍是硬限制**（文档在 `\\wsl.localhost\…`、模板在 `D:\…` 时没有公共祖先）：typst 单根模型决定，CLI 也一样。这种情况由 `escape_hint` 把**当前项目根**与原因追加进诊断（typst 原文只有一句黑话，用户无从下手）。0.4.0 删掉的 wasm 时代"打开时把同目录 .typ 注册进虚拟 FS"那条链路（issue #34 的修法所在）由此彻底退场：相对导入现在全由磁盘解析承担。
...
- **写盘只有一条路（2026-09-16 审计过，用户问「编辑器会清空文件吗？？」）**：`handleSave()` → `saveTypFile()` → Rust `write_file`，而 `handleSave()` 只挂在三处显式保存上（菜单/右键「保存」、`Ctrl+S`、关窗弹窗的「保存并关闭」）。**没有自动保存、没有定时写盘**，所以不按保存时磁盘上的 `.typ` 一个字节都不会动（崩了/断电/被 kill 也一样）。能拿走内容的只有两条，**确认只剩一道**：① **「新建」`Ctrl+N`** —— 它清空编辑器 + 置空 `filePath` + `clearState()` 清会话存档，此前**一句都不问**，现在与「打开…」「`Ctrl+R`」一样先 `confirmDiscard`（磁盘文件仍不受影响：`filePath` 被置空，紧接着按 `Ctrl+S` 走的是另存为）；② **「空文档 + 已有文件 + 按保存」** —— 唯一能把磁盘文件写成空的组合。**这一条的确认窗 2026-09-18 被用户要求删掉了**（原话「删了这个窗口」；删的那个原生对话框标题是「保存空文档」，正文「「x.typ」的内容是空的（只有空白字符），保存会把磁盘上的文件也清空。仍要保存吗？」）—— 现在空文档存进已有文件是**直接写空、不再问**，判定函数 `needsBlankOverwriteConfirm`（连同它的 4 组单测）一并删除。**别再"顺手加回来"**：那是用户看过实际弹窗后明确要删的，理由是他自己知道在存什么。另外两个前提仍在：**启动恢复有安全阀**（只有 `saved.content.trim() !== ""` 才恢复，空内容连 `filePath` 一起不恢复 —— 否则会出现"空内容 + 有文件路径"，一按 `Ctrl+S` 就把好文件清空），以及**验收第 38 组**钉住的"编辑全程 `write_file` 调用次数为 0"。**注意 `fs::write` 是"截断再写"**，不是原子写：进程在写中途被杀/断电理论上会留下空文件或半截文件，暂未改成"临时文件 + rename"。
...
- **浏览器端交互验证（无显示器环境下的验收手段）**：`scripts/browser-check/`（零依赖 CDP 驱动）
  ...
  - `writing-blocks.mjs`：**写作模式块级渲染的验收**（133 项，带 `&blocks=1`）。为什么单独一套：块切片会把非光标块整块换成图片，那块里的 `.cm-markup-heading`、公式 widget 等**在 DOM 里不复存在**（设计如此），`wysiwyg.mjs` 那 290 项断言的是"标记装饰"世界，默认开着块渲染会整片变红、把回归信号淹掉 —— 所以桩里 `compile_blocks` **默认返回"没有这个命令"**（页面自动退回整页预览路径），只有 `&blocks=1` 时才给假切片（见 `browser-dev-stub.ts` 的 `blocksStubEnabled`）。这套验的是：非光标块被切片取代（**按 `.cm-line` 判"还是不是源码形态"** —— 不能用 `.cm-content.textContent`，切片的 SVG 里也有文字，实测踩过）、切片铺满正文列宽、点击切片→光标落到该块起点且原活动块变切片、活动块内可正常输入、`Ctrl+E` 切源码模式无切片且切回恢复（0.8.2 起模式切换从 `Ctrl+/` 改成 `Ctrl+E`，`Ctrl+/` 归注释）、暗色挂 `cm-block-crop-dark`、长文档窗口化后滚动仍补渲出切片。
  - `writing-blocks-visual.mjs`：**块级切片的几何等价验收（真实产物，76 项）**。先 `npm run fixtures:blocks`（Rust `dump_block_fixtures`）导出每块的真实区间/几何/SVG，导航前注入 `window.__DEV_BLOCK_FIXTURES`，桩命中同文档夹具时给真实产物；然后断言"切片摞起来 == 原版式"：每块高度与真实排版一致（实测偏差 **0.01px**）、相邻切片首尾相接（**0.00px**）、总跨度等于真实纵向跨度、铺满正文列宽、高宽比未被拉伸、被盖住的块不在源码形态里。**这条是"和真实 typst 一样"的唯一数字证据**，改块级渲染后必须跑。
...
## 代码审查：**只在"处理 PR"时做这一套**（2026-09-18 PR #60 定下来的）

**什么时候用**：用户丢来一个 PR 链接说"处理一下"、**别的会话 / 别的线**交来的分支要合进 main。这时才跑下面这套。
**什么时候不用**：自己直推 main 的日常改动 —— 那是 `npm run check` + 相关单测（几秒级），改到编辑器/装饰/布局才加跑浏览器验收（见「测试」）。**别把并发子代理那套套到日常小改上**（为 10 行的修复开三个审查者纯属浪费）。判断依据是"这份代码的上下文是不是我自己的"：自己写的自己知道哪里危险，别人那条线的自述与实现是否一致**必须验**。

### 两条腿必须都走，缺一条就会漏（这是本节存在的理由）

`npm run check` / `npm test` / `cargo test` / 五套浏览器验收**全绿**的 PR，仍然可以带着"真机根本走不到"的 bug —— 因为**桩是按契约写的**，真后端不这么发。PR #60 的 `blocks` 键就是活例（桩自己在失败时补了 `blocks: []`，真 Rust 侧被 serde 省略 ⇒ 前端判成"后端不支持"），与 0.8.3 那个 `path: null` 是同一类陷阱。所以：**跑起来证明"断言下的行为对"，读代码找"断言之外真机会怎样"**。

### 腿一：并发子代理「读代码」——按风险面切，边界故意重叠

一次消息里发出去（各自独立上下文、互不可见），**通常三个**，切法不是按目录而是按风险面：

1. **后端 / 引擎**：IPC 契约（成功与失败两种形状）、panic 逃逸、缓存键与生命周期、锁与线程模型、路径穿越、O(n²)。
2. **前端核心**：仓库红线逐条核对、编辑态不变量（旧表 vs 新文档）、选区/焦点语义、每按键的全文档扫描。
3. **集成 / 文档 / 验收脚本**：**会不会压坏上一个发行版的功能**、退回路径、脚本里的断言是不是真断言（有没有 `if (x)` 式静默跳过）、文档数字与代码是否对得上、仓库卫生（删掉的文件真没人引用、没混进大文件/临时文件）。

**边界故意重叠**：同一条 bug 被两个角度分别撞到才算独立证据。PR #60 的 `blocks` 键那条是三个互不可见的审查者从三个入口撞出同一个结论，加上我自己复核 = 四条独立证据。

**每份 prompt 必须自带这六段**（子代理看不到本会话，缺一段结果就不可用）：

- ① **git 现场**：`HEAD` 是哪个提交、基线是 `main` 的哪个提交，并点明"`main` 是 HEAD 的祖先 ⇒ `git diff main HEAD -- <路径>` 就是 PR 改动"。
- ② **PR 是什么 + 它自称做到了什么**（不写这段，模型会去审"设计对不对"，而不是"实现与自述对不对"）。
- ③ **本仓库的红线原文**（`editorDoc` 实时镜像 / 装饰 `b>a` 且必须在 try/catch 内 / 留白挂 `.scroller` / 不许夺焦 / 版本与发版纪律 / 不许动签名密钥）—— **这段最值钱**：把"这个仓库踩过的坑"变成审查清单，而不是让模型自由发挥。
...
- ⑥ **只读纪律**：不许改文件、不许 `git checkout`/`commit`/`stash`；**也不许跑 `npm test` / `cargo` / 浏览器验收** —— 那时主 agent 正在跑腿二的全套，会和它抢 CPU、`src-tauri/target` 与 1425/9333 端口。改文件、提交、跑起来都归主 agent。

### 腿二：主 agent 自己「跑起来」（与腿一并发，不阻塞回合）

照「测试」那节跑全套：`npm run check` → `npm test` → `cargo test` → `npm run fixtures:{blocks,math}` → 五套浏览器验收。
**本机跑法**（一次性脚本 `.browser-check/prXX-verify.sh` 最省事：自己起 dev server + headless chromium、跑完自己关，日志重定向进 `.browser-check/prXX-verify.log`）：`PATH="$HOME/.cargo/bin:$PATH"`、`BROWSER_CHECK_URL='http://127.0.0.1:1425/?browserdev=1'`、Linux chromium 用 Playwright 缓存里那个 `chromium_headless_shell-*`（见「环境备忘」）。跑完把每个套件的**计数**记下来与 PR 自述逐条对（PR #60 就用这招发现"wysiwyg-visual 自述 15、实际 20"）。

### 复核纪律（别把子代理的结论直接转述给用户）

- 最高杀伤力的那几条**自己读代码复核**。PR #60 我亲自确认了 `skip_serializing_if = "Vec::is_empty"` + `fail()` 把 blocks 置空 + 前端形状检查排在 `out.ok` 之前 —— 纯 serde 语义，不用跑就能定死。
- **只出现一次、自己又没复核的，一律降级**（进"一般/建议"，不许写进"严重"）。
- 报告里要分清"**我验过的**"与"**只转述的**"，并且把方法自身的局限写进去：读代码 ≠ 执行证明（要钉死得跑一次真命令）；不是全量逐行审，只深读高风险面；模型会漏也会误报。

### 结论落盘：写到 PR 评论里，不要只留在对话中

`gh pr comment <n> --repo Z3O1/Typst-pad --body "$(cat .browser-check/prXX-comment.md)"`（body 先写成文件再读进变量，中文与换行才不会被 shell 吃掉）。评论里要有：**独立复跑的计数表** + 按严重度排的清单（每条带 `文件:行号` 与最小修法）+ 明确写"哪些类别没发现问题"。理由：那条线/那个人看得到、可追溯；只写在对话里，换个会话就没了。

### 审查与合并的关系（PR #60 用户当场定的口径）

- **审查发现的问题不阻塞合并**，除非是"合了会坏掉**已发布**功能"那一类（新功能线自己的 bug 不是阻塞项：main 上的代码不经过 Release 就影响不到任何用户）。
- **合并方式：多条提交的功能线用 merge commit，单笔小改动 squash 无所谓**。判据是"提交历史本身有没有信息量"：#60 那 30 个提交是这条线的"故事"，文档（CLAUDE.md / 交接文档）按提交号引用它们（`a07f3ad` 这种），squash 会把这些引用打断；#59 只是仓库转公开后的一处文案修正，squash 成 `fd271f7` 一笔正好。
- **合并后不删分支**：那条线还在继续用（它的会话仍在自己目录里工作），删了等于端掉别人的工作台。
- **修复归属由用户定**：清单交回原线、还是我在 main 上修，都要先问（PR #60 用户选的是"交回那条线，先不动"）。
- 审查与合并都**不碰版本号 / CHANGELOG / tag / Release**（红线 11 照旧：发版权在用户手里）。
