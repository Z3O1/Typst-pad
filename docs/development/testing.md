...
- **折行位置由浏览器决定，与 Typst 的断行算法不完全一致**：纵向几何已精确对齐，但"折成几行"另有来源（Typst 全局最优 + CJK 标点压缩/悬挂 vs Chromium 贪心）。四份作业 210 个可编辑块里 6 块差 1 行（两个方向都有），当前如实记为已知边界，修法是让引擎回传行断点（见写作渲染的说明）。
