...
- **折行位置由引擎断点决定**（2026-09-25 起）：编辑器按 `BlockCrop::line_breaks` 强制换行并禁掉行内折行（见写作渲染的「折行」一节）。四份作业 210 个可编辑块里 205 块与 Typst 逐行相等；剩 5 块（数分周二 L48/L54/L72/L122/L154）的断点落在**行内公式内部** —— 浏览器折不了原子 widget，整块退回贪心折行，如实记为已知边界。别拿"引擎给了断点"当理由放宽这条判据：断点必须与 `lineCount` 自洽、且有一枚落不上就整块不折，否则会出现"行数反而更多"的假绿（实测高代周二 L207 4→6 行）。
