...
- CI（GitHub Actions，`.github/workflows/ci.yml`）：
  - `test`（ubuntu）：push 到 main / PR 时跑 类型检查 → 单测 → 前端构建 → `cargo check`
  - `build-bundles`（windows）：同样在 main / PR 触发，构建 .exe/.msi 安装包并 `upload-artifact`（main 触发时吃 Rust 缓存）
...
2. 提交推送并合并到 `main` → CI 自动构建安装包并上传 artifact（命中 Rust 缓存，快速）
3. 打 tag 触发发布：`git tag v0.2.x && git push origin v0.2.x`
4. GitHub → Actions → **Release**：下载最新 artifact → 生成草稿 Release（自动上传安装包）
5. Releases 页面编辑草稿 → 发布

> 缓存机制：`Swatinem/rust-cache` 的 key 基于 rust 版本 + `Cargo.lock` 哈希。安装包在 main 分支构建（scope=main），依赖不变（Cargo.lock 不变）时跨版本命中；发布 workflow 不安装依赖、不需要缓存。
