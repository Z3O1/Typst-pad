...
- 写作模式**块级渲染**的验收（需要 headless Chromium，见脚本头部注释）：`writing-blocks.mjs`（133 项，桩产物：切片/展开/窗口化/竖直移动/点击锚定/翻页/编译失败/各种输入）、`writing-blocks-visual.mjs`（67 项，`npm run fixtures:blocks` 导出真实切片后验几何等价 + 链接热区）、`writing-blocks-hit.mjs`（28 项 / 136 次点击全中，点击 → 精确字符）、`writing-mode-scenes.mjs`（65 项，9 篇场景 + 截图）
