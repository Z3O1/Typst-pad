...
// 三个 `$state`）。依赖全注入（`zoom-controller` / `math-queue` / `document-session` 同一套路），
// 单测里可以拿假 invoke 把两条容易写错的规则钉死：
//
// 1. **列出来的族名来自 Rust 真实注册的字体**（结构上不可能在下拉里写出一个不存在的族名），
//    所以扫描用的目录必须先归一化（trim / 去空 / 去重）——同一个目录写两遍会让引擎扫两遍。
// 2. **默认族列表取一次就够**（静态），拿不到时**不许用空数组覆盖**已经拿到的那份，否则
//    "选中项 + 其余兜底"的兜底部分会被清空（用户会看到生僻字掉回楷体/日文字形）。
//
