...
npm test             # 单测（49 文件 / 925 项）；npm test -- <文件> 跑单个
...
- **文件/安全（→02）**：写盘只有一条路（`file-flow` 的 `save → saveTypFile → write_file`），**没有自动保存**；**绝不能让 `editorDoc` 落后**；`validate_typ_path` / `validate_write_path` 不许绕过；`csp` 保持 `null`；「空文档存进已有文件」**不弹确认窗、直接写空（勿加回）**。
